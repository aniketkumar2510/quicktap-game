...
export default function Home() {
  ...
  const [deleteDialogPlayers, setDeleteDialogPlayers] = useState<string[]>([]);
  const [bulkDeletePlayers, setBulkDeletePlayers] = useState<string[]>([]);
  ...
  const openDeleteDialog = (names: string[]) => {
    ...
    if (uniqueNames.length) setDeleteDialogPlayers(uniqueNames);
  };

  const confirmDeletePlayers = () => {
    const names = deleteDialogPlayers;
    if (!names.length) return;
    const lowerNames = new Set(names.map((name) => name.trim().toLowerCase()));
    setSessions((current) => current.filter((session) => !lowerNames.has(session.winner.trim().toLowerCase())));
    setPlayerScores((current) => Object.fromEntries(Object.entries(current).filter(([name]) => !lowerNames.has(name.trim().toLowerCase()))));
    if (lowerNames.has(activePlayerName.trim().toLowerCase())) {
      setPlayerName("Alex");
      setSessionNickname("Alex");
      setRoundTimes([]);
      setReactionTime(null);
      setRound(1);
      setGameState("idle");
    ...
    setHistoryPlayer("");
    setBulkDeletePlayers([]);
    setDeleteDialogPlayers([]);
    toast.success(names.length === 1 ? "Player deleted" : "Players deleted", { description: `${names.join(", ")} and all related saved data were removed.` });
  };
  ...
  const typedNickname = playerName.trim();
  const nicknameTaken = Boolean(typedNickname && typedNickname.toLowerCase() !== sessionNickname.trim().toLowerCase() && historyPlayerOptions.some((name) => name.toLowerCase() === typedNickname.toLowerCase()));
